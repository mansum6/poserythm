...
punchMinExtensionRatio: 0.5,           // How extended the arm must be (0.8 = 80% extension toward 90 degrees)
...
export function interpretPose(rawKeypoints, externalDebugUpdater = null) {
  ...
  // Calculate punch confidences instead of binary detection
  const leftPunchConfidence = _calculatePunchConfidence(lw, le, ls, true, shoulderMidY, shoulderWidth);
  const rightPunchConfidence = _calculatePunchConfidence(rw, re, rs, false, shoulderMidY, shoulderWidth);

  if (externalDebugUpdater) {
    externalDebugUpdater('Left Punch Confidence', leftPunchConfidence.toFixed(3));
    externalDebugUpdater('Right Punch Confidence', rightPunchConfidence.toFixed(3));
  }
  ...
  if (leftPunchConfidence >= POSE_DETECTION_CONFIG.punchConfidenceThreshold) {
    return POSE_STATES.PUNCH_LEFT;
  }
  if (rightPunchConfidence >= POSE_DETECTION_CONFIG.punchConfidenceThreshold) {
